...
{
    /*
    * module: mhsc_clubloulous
    * date: 2021-06-17 13:14:24
    * version: 0.1.0
    */
    public function sendEmail($order, $template_vars = false)
    {
        ...
        if (isset($result['template']) && Validate::isEmail($result['email'])) {
            ...
            if($result['id_order_state'] == 20 ) {
                $Products = Db::getInstance()->executeS('SELECT id_product, quantity FROM '. _DB_PREFIX_ .'cart_product WHERE id_cart ='. $order->id_cart);
            ...
            if($result['id_order_state'] == 21 ) {

                $Products = Db::getInstance()->executeS('SELECT id_product, quantity FROM '. _DB_PREFIX_ .'cart_product WHERE id_cart ='. $order->id_cart);
                
                foreach ($Products as $product) {
                    if($product['id_product'] == 3416)
                    {
                        $data = array_merge($data, ['{reference}' => $order->reference]);
                        $data = array_merge($data, ['{idclient}' => $order->id_customer]);

                        $context = Context::getContext();

                        $lien = $context->link->getModuleLink('mhsc_clubmembre', 'clubmembre');

                        $data = array_merge($data, ['{lienfrontcontroller}' => $lien ]);
                    }
                }
            }
